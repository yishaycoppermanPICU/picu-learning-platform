...
# -*- coding: utf-8 -*-
"""
סקריפט להוספת מבחן Central Line למסד הנתונים
"""
...
def main():
    print("=" * 80)
    print("🧪 הוספת מבחן Central Line Management למערכת")
    print("=" * 80)
    ...
    # מחפש את הנושא Central Line
    print("\n🔍 מחפש את נושא Central Line Management...")
    topics = get_topics()
    
    content_item_id = None
    for topic in topics:
        if 'central' in topic.get('slug', '').lower() or 'צנתר' in topic.get('title', ''):
            content_item_id = topic['id']
            print(f"✅ נמצא Central Line! ID: {content_item_id}")
            break
    
    if not content_item_id:
        print("❌ לא נמצא נושא Central Line! יש ליצור אותו קודם.")
        return
    
    # טוען שאלות מקובץ
    print("📂 טוען שאלות מקובץ...")
    with open('central_line_quiz.json', 'r', encoding='utf-8') as f:
    ...
    if success_count == len(questions):
        print("\n🎉 המבחן נוסף בהצלחה!")
        print("\n💡 כעת ניתן:")
        print("   1. להיכנס לדף התוכן של Central Line Management")
