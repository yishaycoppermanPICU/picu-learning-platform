...
def add_pancytopenia_quiz():
    ...
    # מציאת ה-content_item_id של פאנציטופניה
    print("\n🔍 מחפש את נושא פאנציטופניה...")
    topics = get_topics()
    pancytopenia_topic = None
    
    for topic in topics:
        if topic.get('slug') == 'pancytopenia':
            pancytopenia_topic = topic
            break
    
    if not pancytopenia_topic:
        print("❌ לא נמצא נושא פאנציטופניה במסד הנתונים")
        print("💡 יש להריץ את add_pancytopenia.py קודם")
        return False
    
    content_item_id = pancytopenia_topic['id']
    print(f"✅ נמצא פאנציטופניה! ID: {content_item_id}")
    
    # טעינת השאלות מקובץ JSON
    print("\n📂 טוען שאלות מקובץ...")
    with open('pancytopenia_quiz.json', 'r', encoding='utf-8') as f:
...
if __name__ == "__main__":
    print("="*50)
    print("🩸 הוספת מבחן פאנציטופניה למערכת")
    print("="*50)
    
    success = add_pancytopenia_quiz()
    
    if success:
        print("\n🎉 המבחן נוסף בהצלחה!")
        print("\n💡 כעת ניתן:")
        print("   1. להיכנס לדף התוכן של פאנציטופניה")
