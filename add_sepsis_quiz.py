...
# -*- coding: utf-8 -*-
"""
סקריפט להוספת מבחן Sepsis למסד הנתונים
"""
...
def main():
    print("=" * 80)
    print("🧪 הוספת מבחן Sepsis Protocol למערכת")
    print("=" * 80)
    ...
    # מחפש את הנושא Sepsis
    print("\n🔍 מחפש את נושא Sepsis Protocol...")
    topics = get_topics()
    
    content_item_id = None
    for topic in topics:
        if 'sepsis' in topic.get('slug', '').lower() or 'ספסיס' in topic.get('title', ''):
            content_item_id = topic['id']
            print(f"✅ נמצא Sepsis! ID: {content_item_id}")
            break
    
    if not content_item_id:
        print("❌ לא נמצא נושא Sepsis! יש ליצור אותו קודם.")
        return
    
    # טוען שאלות מקובץ
    print("📂 טוען שאלות מקובץ...")
    with open('sepsis_quiz.json', 'r', encoding='utf-8') as f:
    ...
    if success_count == len(questions):
        print("\n🎉 המבחן נוסף בהצלחה!")
        print("\n💡 כעת ניתן:")
        print("   1. להיכנס לדף התוכן של Sepsis Protocol")
