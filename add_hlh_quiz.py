...
def add_hlh_quiz():
    """הוספת 10 שאלות על HLH למסד הנתונים"""
    ...
    # מציאת ה-content_item_id של HLH
    print("\n🔍 מחפש את נושא HLH...")
    topics = get_topics()
    hlh_topic = None
    
    for topic in topics:
        if topic.get('slug') == 'hlh_syndrome':
            hlh_topic = topic
            break
    
    if not hlh_topic:
        print("❌ לא נמצא נושא HLH במסד הנתונים")
        print("💡 יש להריץ את add_hlh.py קודם")
        return False
    
    content_item_id = hlh_topic['id']
    print(f"✅ נמצא HLH! ID: {content_item_id}")
    
    # טעינת השאלות מקובץ JSON
    print("\n📂 טוען שאלות מקובץ...")
    with open('hlh_quiz.json', 'r', encoding='utf-8') as f:
...
if __name__ == "__main__":
    print("="*50)
    print("🔬 הוספת מבחן HLH למערכת")
    print("="*50)
    
    success = add_hlh_quiz()
    
    if success:
        print("\n🎉 המבחן נוסף בהצלחה!")
        print("\n💡 כעת ניתן:")
        print("   1. להיכנס לדף התוכן של HLH")
