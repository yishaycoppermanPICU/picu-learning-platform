...
# -*- coding: utf-8 -*-
"""
סקריפט להוספת מבחן Hypovolemic Shock למסד הנתונים
"""
...
def main():
    print("=" * 80)
    print("🧪 הוספת מבחן Hypovolemic Shock למערכת")
    print("=" * 80)
    ...
    # מחפש את הנושא
    print("\n🔍 מחפש את נושא Hypovolemic Shock...")
    topics = get_topics()
    
    content_item_id = None
    for topic in topics:
        if 'hypo' in topic.get('slug', '').lower() or 'היפו' in topic.get('title', '') or 'המור' in topic.get('title', ''):
            content_item_id = topic['id']
            print(f"✅ נמצא! ID: {content_item_id}")
            print(f"   כותרת: {topic.get('title')}")
            break
    
    if not content_item_id:
        print("❌ לא נמצא נושא! יש ליצור אותו קודם.")
        return
    
    # טוען שאלות מקובץ
    print("📂 טוען שאלות מקובץ...")
    with open('hypovolemic_shock_quiz.json', 'r', encoding='utf-8') as f:
    ...
    if success_count == len(questions):
        print("\n🎉 המבחן נוסף בהצלחה!")
        print("\n💡 כעת ניתן:")
        print("   1. להיכנס לדף התוכן של Hypovolemic Shock")
